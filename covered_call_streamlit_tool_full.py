...
if api_key and us_api_symbol:
    ...
    if api_key and us_api_symbol and dates:
        ...
        if calls:
            ...
            # Strike-Empfehlung basierend auf Risiko/Rendite
            df_calls["strike"] = df_calls["strike"].astype(float)
            df_calls["bid"] = df_calls["bid"].astype(float)
            df_calls["distance_to_strike"] = (df_calls["strike"] - float(chain_data["optionChain"]["result"][0]["quote"]["regularMarketPrice"]))
            df_calls["risk_factor"] = (df_calls["bid"] / df_calls["distance_to_strike"].abs()).replace([float("inf"), -float("inf")], 0).fillna(0)

            recommended_strike = df_calls.loc[df_calls["risk_factor"].idxmax()]
            st.markdown("---")
            st.markdown("💡 **Strike-Empfehlung (Rendite vs. Risikodistanz)**")
            st.write(f"Empfohlener Strike: **{recommended_strike['strike']}** mit Prämie: **{recommended_strike['bid']} USD**")
            st.write(f"Risikodistanz: {recommended_strike['distance_to_strike']:.2f} USD | Verhältnis (Prämie/Distanz): {recommended_strike['risk_factor']:.2f}")

            # IV-Rank-Schätzung (Simulation)
            iv_current = chain_data.get("optionChain", {}).get("result", [{}])[0].get("quote", {}).get("impliedVolatility", 0)
            if iv_current:
                iv_rank = "🔼 Hoch" if iv_current > 0.30 else ("⚖️ Neutral" if iv_current > 0.20 else "🔽 Niedrig")
                st.markdown(f"📊 **IV-Rank (geschätzt):** {iv_rank} ({iv_current:.2%})")
